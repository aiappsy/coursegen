...
const SharedCoursePage = () => {
    ...
    const { mainTopic, type, courseId, end, pass, lang, v } = state || {};
    ...
    const [courseRole, setRole] = useState('');
        ...
        if (!isLoading) {
            ...
            if (mSubTopic.theory === '' || mSubTopic.theory === undefined || mSubTopic.theory === null) {
                if (type === 'video & text course') {
                    ...
                    const prompt = `Strictly in ${lang}, Explain me about this subtopic of ${mainTopic} with examples :- ${mSubTopic.title}. Please Strictly Don't Give Additional Resources And Images.  Do not include phrases such as "Okay", "Sure", or "I'll explain". Start directly with the explanation. Write in a professional, documentation style.`;
    ...
    async function generateQuiz() {
        if (!isLoading) {
            ...
            const response = await axios.post(postURL, { courseId, mainTopic, subtopicsString, lang, noOfQuestions });
            if (response.data.success) {
                setIsLoading(false);
                                        ...
                                        onClick={() => {
                                            if (v === '2') {
                                                ...
                                                toast({
                                                    ...
                                                    description: "You currently have view-only access to this course. Please contact the course admin to request full access.",
                                                });
                                            }
                                        }}
                                        key={subtopic.title}
    ...
    return (
        <div className="flex flex-col h-screen bg-background overflow-hidden">
            ...
            <Button onClick={() => setIsShareOpen(true)} variant="ghost" size="sm" asChild>
            ...
            {isQuizOpen ?
                <Dialog open={isQuizOpen} onOpenChange={setIsQuizOpen}>
                    <DialogContent className="sm:max-w-[425px]">
                        <DialogHeader>
                            <DialogTitle>No. Of Questions</DialogTitle>
                            <DialogDescription>
                                Select No. Of Questions For Quiz
                            </DialogDescription>
                        </DialogHeader>
                        <div>
                            <Select onValueChange={setNoOfQuestions}>
                                <SelectTrigger>
                                    <SelectValue placeholder="Select No. Of Questions" />
                                </SelectTrigger>
                                <SelectContent>

                                    {noQuestions.map((item) => (
                                        <SelectGroup>
                                            <SelectItem onClick={() => setNoOfQuestions(item.value)} value={item.value}>{item.value}</SelectItem>
                                        </SelectGroup>
                                    ))}
                        ...
                        <DialogFooter>
                            <DialogClose asChild>
                                <Button onClick={() => setIsQuizOpen(false)} variant="outline">Cancel</Button>
                            </DialogClose>
            ...
            {isShareOpen ?
                <Dialog open={isShareOpen} onOpenChange={setIsShareOpen}>
                    <DialogContent className="sm:max-w-[425px]">
                        ...
                        <div>
                            <Select onValueChange={setRole}>
                                ...
                                <SelectContent>
                                    ...
                                    <SelectGroup>
                                        <SelectItem onClick={() => setRole('1')} value="1">View Only</SelectItem>
                                        <SelectItem onClick={() => setRole('2')} value="2">All Access</SelectItem>
                                    </SelectGroup>
                        ...
                        <DialogFooter>
                            <DialogClose asChild>
                                <Button onClick={() => setIsShareOpen(false)} variant="outline">Cancel</Button>
                            </DialogClose>
                            <ShareOnSocial
                                textToShare={sessionStorage.getItem('mName') + " shared you course on " + mainTopic}
                                link={websiteURL + '/shareable?id=' + courseId + '&v=' + courseRole}
                                ...
                                noReferer
                            >
                                <Button onClick={() => setIsShareOpen(false)}>Share</Button>
                            </ShareOnSocial>
                        </DialogFooter>
                    </DialogContent>
                </Dialog>
