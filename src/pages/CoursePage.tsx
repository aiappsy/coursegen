...
const CoursePage = () => {
  ...
  async function htmlDownloadPPT() {
    ...
    // 1. Cover Slide
    pptx.addSlide().addText(mainTopic, {
      x: 0.5,
      y: 2,
      fontSize: 36,
      bold: true,
    ...
    // indexSlide.addText('Index', { x: 0.5, y: 0.5, fontSize: 28, bold: true });

    // let indexY = 1.2;
    // topics.forEach(topic => {
    //   indexSlide.addText(topic.title, {
    //     x: 0.7,
    //     y: indexY,
    //     fontSize: 20,
    //     bullet: true
    ...
    const slideMaxHeight = 5.5; // max Y value before needing new slide
    const topicSpacing = 0.4;
    const subtopicSpacing = 0.3;
    const topMargin = 0.5;

    // Create first index slide
    let indexSlide = pptx.addSlide();
    indexSlide.addText('Index', {
      x: 0.5, y: topMargin,
      fontSize: 28, bold: true
    });
    let indexY = topMargin + 0.7;

    topics.forEach(topic => {
      // If not enough space for topic + at least 1 subtopic, start new slide
      if (indexY + topicSpacing + subtopicSpacing > slideMaxHeight) {
        indexSlide = pptx.addSlide();
        indexSlide.addText('Index (contd.)', {
          x: 0.5, y: topMargin,
          fontSize: 28, bold: true
        });
        indexY = topMargin + 0.7;
      }
      ...
      topic.subtopics.forEach(subtopic => {
        // If no space left for subtopic, create a new slide
        if (indexY + subtopicSpacing > slideMaxHeight) {
          indexSlide = pptx.addSlide();
          indexSlide.addText('Index (contd.)', {
            x: 0.5, y: topMargin,
            fontSize: 28, bold: true
          ...
          // Repeat topic title for continuity
          indexSlide.addText(topic.title, {
            x: 0.5,
            y: indexY,
            fontSize: 20,
        ...
        indexSlide.addText(`- ${subtopic.title}`, {
          x: 0.8,
          y: indexY,
          fontSize: 16,
          color: '444444'
        });
        indexY += subtopicSpacing;
      });

      // Extra spacing after topic block
      indexY += 0.1;
    });
    ...
    for (const topic of topics) {
      // Topic Title Slide
      pptx.addSlide().addText(topic.title, {
        x: 0.5,
        y: 1.5,
        fontSize: 28,
        bold: true,
      ...
      for (const subtopic of topic.subtopics) {
        ...
        // if (type === 'text & image course') {
        //   slide.addText('View example image ' + subtopic.image, {
        //     x: 0.5,
        //     y: 0.5,
        //     fontSize: 20,
        //     color: "2a81fa",
        ...
        //   contentY += 3.5;
        // } else {
        //   slide.addText('View example video ' + subtopic.youtube, {
        //     x: 0.5,
        //     y: 0.5,
        //     fontSize: 20,
        //     color: "2a81fa",
        ...
        if (subtopic.done) {
          ...
          if (type === 'text & image course' && subtopic.image) {
            const imageData = await toDataUrl(subtopic.image);
            if (imageData) {
              slide.addImage({ path: imageData, x: 0.5, y: contentY, w: 5, h: 3 });
              contentY += 3.5;
            } else {
              slide.addText("View example image", {
                x: 0.5, y: contentY,
                color: "0000FF",
                underline: true,
                fontSize: 18,
                hyperlink: { url: subtopic.image }
              });
              contentY += 0.1;
            }
          } else {
            slide.addMedia({
              type: "online",
              link: 'https://www.youtube.com/embed/' + subtopic.youtube,
              x: 0.5, y: contentY, w: 5, h: 3
            });
            contentY += 3.5;
          }

          // Add theory content (HTML → Text → Split)
          // if (subtopic.theory) {
          //   const plainText = stripHtml(subtopic.theory);
          //   const chunks = splitTextIntoChunks(plainText, 800); // tune if needed

          //   chunks.forEach((chunk, index) => {
          //     const theorySlide = index === 0 ? slide : pptx.addSlide();
          //     if (index > 0) {
          //       theorySlide.addText(`${subtopic.title} (contd.)`, {
          //         x: 0.5,
          //         y: 0.5,
          //         fontSize: 20,
          //         bold: true
          ...
          //     theorySlide.addText(chunk, {
          //       x: 0.5,
          //       y: index === 0 ? contentY : 1.2,
          //       fontSize: 14,
          //       w: '85%',
          //       h: 5.5,
          //       valign: 'top',
          ...
          if (subtopic.theory) {
            ...
            // Determine space for the first theory slide
            let firstSlideSpace;
            if (type === 'text & image course') {
              firstSlideSpace = imageData ? 1 : 5;
            } else {
              firstSlideSpace = 1;
            }

            const restSlideSpace = 5;

            const firstSlideLines = Math.floor(firstSlideSpace / lineHeight);
            const restSlideLines = Math.floor(restSlideSpace / lineHeight);

            const firstChunk = lines.slice(0, firstSlideLines).join("\n");
            const remainingLines = lines.slice(firstSlideLines);

            const remainingChunks = [];
            for (let i = 0; i < remainingLines.length; i += restSlideLines) {
              remainingChunks.push(remainingLines.slice(i, i + restSlideLines).join("\n"));
            }
          ...
          slide.addText(`Please visit ${subtopic.title} topic to export.`, {
            x: 0.5,
            y: 1.5,
            fontSize: 14,
          });
    ...
    // Download the PPT
    pptx.writeFile({ fileName: `${mainTopic}.pptx` });
    setExporting(false);
    setIsExportOpen(true);
  }

  // Utility: convert HTML string to plain text (basic version)
  ...
  // Utility: split long text into slide-friendly chunks
  function splitTextIntoChunks(text, maxChars = 1000) {
    const chunks = [];
    while (text.length > 0) {
      let chunk = text.substring(0, maxChars);

      // Try to break at sentence or word boundary
      let lastPeriod = chunk.lastIndexOf(".");
      let lastSpace = chunk.lastIndexOf(" ");
      let splitAt = lastPeriod > 200 ? lastPeriod + 1 : lastSpace;
                    ...
                    <div className="space-y-4">
                      {type === 'video & text course' ?
                        ...
                        <div className="relative group w-fit">
                          {!imgError && (
                            ...
                            >
                              <img
                                alt="Media"
                                src={media}
                                className="overflow-hidden h-96 max-md:h-64 rounded-xl shadow-md transition-transform duration-300 group-hover:opacity-90 cursor-pointer"
                          ...
                          <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-black/20 rounded-xl pointer-events-none">
                            <div className="bg-white/80 rounded-full p-3 shadow-md transform scale-90 group-hover:scale-100 transition-transform duration-300">
      ...
      <Dialog open={isExportOpen} onOpenChange={setIsExportOpen}>
        <DialogContent className="sm:max-w-lg rounded-2xl shadow-lg">
          ...
          <div className="flex flex-col space-y-4">
            {/* Export PPT */}
            <button
              onClick={() => htmlDownloadPPT()}
              className="group flex items-center border border-gray-200 dark:border-gray-500 hover:border-primary/60 hover:bg-primary/5 transition-all duration-300 py-5 px-4 rounded-xl"
            >
              <div className="p-3 bg-primary/10 rounded-lg group-hover:bg-primary/20 transition-colors">
                <PresentationIcon size={26} className="text-primary" />
              </div>
              <div className="ml-4 text-left">
                <p className="font-semibold text-gray-800 dark:text-white group-hover:text-primary transition-colors">
                  {exporting ? 'Exporting...' : 'Export as PPT'}
                </p>
                <p className="text-sm text-gray-500">Download course as a PowerPoint file</p>
              </div>
            </button>
            ...
            >
              ...
              <div className="ml-4 text-left">
                ...
                <p className="text-sm text-gray-500">Download the course as a PDF document</p>
              </div>
            </button>
          </div>
        </DialogContent>
