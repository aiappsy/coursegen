...
const Shareable = () => {
    ...
    async function getDataFromDatabase(id) {
        ...
        try {
            ...
            if (sessionStorage.getItem('uid') === null) {
                ...
                navigate('/share/' + id, { state: { jsonData: jsonData, mainTopic: mainTopic.toUpperCase(), type: type, courseId: id, end: '', pass: false, v: '2' } });
            } else {
                ...
                if (responses.data.success) {
                    ...
                    sessionStorage.setItem('jsonData', JSON.stringify(jsonData));
                    navigate('/course/' + id, { state: { jsonData: jsonData, mainTopic: mainTopic.toUpperCase(), type: type.toLowerCase(), courseId: responses.data.courseId, end: '', pass: false } });
                } else {
                    sessionStorage.setItem('shared', id);
                    navigate('/course/' + id, { state: { jsonData: jsonData, mainTopic: mainTopic.toUpperCase(), type: type, courseId: id, end: '', pass: false } });
                }
            }
        } catch (error) {
            showToast("Course doesn't exist");
    ...
    return (
        <div className="text-center h-screen w-screen flex items-center justify-center">
            <Spinner size="xl" className='fill-black text-white dark:text-black dark:fill-white' />
        </div>
    );
};
