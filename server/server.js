...
app.get('/api/coursesaudio', async (req, res) => {
    try {
        const { userId, page = 1, limit = 9 } = req.query;
        ...
        // 2️⃣ Get language docs for these courses
        const courseIds = courses.map(c => String(c._id));

        const langs = await LangSchema.find({
            course: { $in: courseIds }
        });

        // 3️⃣ Build map: courseId -> language
        const langMap = {};
        langs.forEach(l => {
            langMap[l.course] = l.lang?.trim().toLowerCase();
        });

        // 4️⃣ Filter courses by allowed language
        const filteredCourses = courses.filter(course => {
            const lang = langMap[String(course._id)];
            return lang && ALLOWED_LANGS.includes(lang);
        });
